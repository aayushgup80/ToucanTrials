...
class PlayerController {
    ...
    update(dt, input) {
        ...
        if (!this.isDashing) {
            ...
            this.velocity.x = MathUtils.moveTowards(
                this.velocity.x,
                targetVelX,
                accel * dt
            );

            this.velocity.z = MathUtils.moveTowards(
                this.velocity.z,
                targetVelZ,
                accel * dt
            );
        ...
        // ----------------------------------------------------
        // ROTATION (Smoothly turn player to face move direction)
        // ----------------------------------------------------
        if (Math.abs(this.velocity.x) > 0.1 || Math.abs(this.velocity.z) > 0.1) {
            const targetRotation = Math.atan2(this.velocity.x, this.velocity.z);
            this.mesh.rotation.y = MathUtils.lerpAngle(
        ...
        // ----------------------------------------------------
        // FALL BOUNDARY / RESPAWN
        // ----------------------------------------------------
        if (this.mesh.position.y < this.fallLimit) {
            this.respawn();
        }
    }
    ...
    getMovementDirection(input) {
        ...
        // Extract Camera Forward and Right directions on the horizontal XZ plane
    ...
    updateState() {
        if (this.isDashing) {
            ...
            this.state = "FALLING";
        } else if (Math.abs(this.velocity.x) > 0.5 || Math.abs(this.velocity.z) > 0.5) {
            this.state = "RUNNING";
        } else {
            this.state = "IDLE";
        }
